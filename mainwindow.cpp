    ...
    : QMainWindow(parent), currentUserId(userId), currentUserName(userName), currentUserEmail(userEmail)
{
    ...
    // Whenever the Route Finder tab is opened, quietly pre-fill the
    // destination based on whatever trek is currently selected.
...
void MainWindow::refreshProfileTripSummary()
{
    ...
    if (!bookedTrip.hasTrek) {
        profileTripLabel->setText("No trip booked yet - plan one in Budget Calculator, then book it.");
        return;
    }

    QString text = QString("<b>%1</b> &middot; <span style='color:#4fb8a2;'>&#10003; Booked</span><br>%2 day(s), %3 people")
                       .arg(QString::fromStdString(bookedTrip.trek.name))
                       .arg(bookedTrip.days)
                       .arg(bookedTrip.people);

    if (!bookedTrip.spots.empty()) {
        text += "<br><br>Spots included:<br>";
        for (const TouristSpot &s : bookedTrip.spots)
            text += QString::fromStdString(s.name) + "<br>";
    }

    if (bookedTrip.hasCalculatedCost)
        text += QString("<br><b>Total: %1 NPR</b>").arg(bookedTrip.totalCost, 0, 'f', 0);
...
{
    if (!bookedTrip.hasTrek) {
        tripBarLabel->setText("No trip booked yet - plan one in Budget Calculator, then book it.");
        refreshProfileTripSummary();
        return;
    }

    QString text = QString("Your Trip: %1  ·  %2 day(s)  ·  %3 people  ·  "
    ...
    if (bookedTrip.hasCalculatedCost)
        text += QString("  ·  ~%1 NPR").arg(bookedTrip.totalCost, 0, 'f', 0);
...
QWidget* MainWindow::setupSpotsTab()
{
    ...
    QLabel *hint = new QLabel("Tip: you'll pick which of these spots to visit later, in the Budget Calculator.");
...
// ══════════════════════════════════════════════════════════════
bool MainWindow::isSpotReachableFromTrek(const TouristSpot &spot, const TrekRoute &trek)
{
    if (spot.location == "Kathmandu" || spot.location == "Chitwan")
        return true;

    string route;
    for (const string &cp : trek.checkpoints)
        route += cp + " | ";

    auto has = [&route](const string &keyword) {
        return route.find(keyword) != string::npos;
    };

    if (spot.location == "Pokhara")
        return has("Pokhara");
    if (spot.location == "Rupandehi")
        return has("Pokhara");
    if (spot.location == "Solukhumbu")
        return has("Lukla") || has("Namche") || has("EBC");
    if (spot.location == "Kaski")
        return has("Ghorepani") || has("Chame") || has("Manang") || has("Pisang");
    if (spot.location == "Rasuwa")
        return has("Langtang") || has("Syabrubesi") || has("Kyanjin");
    if (spot.location == "Mustang")
        return has("Jomsom") || has("Muktinath") || has("Kagbeni") || has("Lo Manthang");

    return false;
}

void MainWindow::refreshBudgetSpotsList()
{
    budgetSpotsList->clear();

    int trekIndex = budgetTrekCombo->currentIndex();
    if (trekIndex < 0 || trekIndex >= (int)backend.trekRoutes.size())
        return;

    const TrekRoute &selectedTrek = backend.trekRoutes[trekIndex];

    for (const TouristSpot &s : backend.touristSpots) {
        if (!isSpotReachableFromTrek(s, selectedTrek))
            continue;

        QListWidgetItem *item = new QListWidgetItem(
            QString::fromStdString(s.name) + "  (" + QString::number(s.entryFee, 'f', 0) + " NPR)");
        item->setFlags(item->flags() | Qt::ItemIsUserCheckable);
        item->setCheckState(Qt::Unchecked);
        item->setData(Qt::UserRole, s.spotId);
        budgetSpotsList->addItem(item);
    }
}
...
QWidget* MainWindow::setupBudgetTab()
{
    ...
    budgetSpotsList = new QListWidget;
    refreshBudgetSpotsList();

    connect(budgetTrekCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &MainWindow::refreshBudgetSpotsList);
    ...
    form->addRow("Tourist spots to visit:", budgetSpotsList);
    ...
    form->addRow("Your budget: NPR", budgetLimitSpin);
...
void MainWindow::onCalculateBudget()
{
    ...
    std::vector<TouristSpot> selectedSpots;
    for (int i = 0; i < budgetSpotsList->count(); i++) {
        QListWidgetItem *item = budgetSpotsList->item(i);
        if (item->checkState() != Qt::Checked) continue;

        int spotId = item->data(Qt::UserRole).toInt();
        for (const TouristSpot &s : backend.touristSpots) {
            if (s.spotId == spotId) {
                selectedSpots.push_back(s);
                break;
            }
        }
    }
    ...
    currentTrip.spots = selectedSpots;
    ...
    proceedToBookingBtn->setEnabled(true);
    ...
    html += QString("<tr><td>Tourist spot entry fees</td><td align='right'>%1 NPR</td></tr>").arg(b.entryFees, 0, 'f', 0);
        ...
        if (b.totalNPR <= budgetLimit) {
            ...
            if (!affordable.empty()) {
                html += "<p style='color:#888891; font-size:11px;'>Treks that would likely fit "
                        "(same days/accommodation/people, before tourist spot fees):</p>";
...
QWidget* MainWindow::setupRouteTab()
{
    ...
    form->addRow("Max budget: NPR", routeBudgetSpin);
...
void MainWindow::onSaveItinerary()
{
    ...
    text += "\nTourist spots included:\n";
    if (currentTrip.spots.empty()) {
        text += "  (none selected)\n";
    } else {
        for (const TouristSpot &s : currentTrip.spots)
...
QWidget* MainWindow::setupBookingsTab()
{
    ...
    guidesTable->setColumnCount(6);
    guidesTable->setHorizontalHeaderLabels(
        {"Name", "License", "Languages", "Experience (yrs)", "Rating", "Contact"});
    ...
    QLabel *reviewHint = new QLabel("Note: you can only review treks you've actually booked.");
    reviewHint->setStyleSheet("color: #888891; font-size: 11px;");

    reviewsLayout->addLayout(reviewTrekRow);
    reviewsLayout->addWidget(reviewsTable);
    reviewsLayout->addLayout(addReviewForm);
    reviewsLayout->addWidget(reviewHint);
...
void MainWindow::refreshAvailableGuideCombo()
{
    ...
    for (const Guide &g : available)
        assignGuideCombo->addItem(QString::fromStdString(g.name), g.guideId);
}
...
void MainWindow::onCreateBooking()
{
    ...
    if (result.first == -1) {
        ...
        for (const TrekRoute &t : backend.trekRoutes) {
            if (t.trekId == trekId) {
                bookedTrip.hasTrek = true;
                bookedTrip.trek = t;
                bookedTrip.days = t.daysRequired > 0 ? t.daysRequired : 7;
                bookedTrip.people = people;
                bookedTrip.hasCalculatedCost = true;
                bookedTrip.totalCost = result.second;
                bookedTrip.isBooked = true;
                bookedTrip.spots = useCalculatedCost ? currentTrip.spots : std::vector<TouristSpot>();
                break;
            }
        }
        updateTripBar();
...
void MainWindow::onAssignGuide()
{
    ...
    QString message;
    bool ok = mysqlManager.assignGuide(bookingId, guideId, message);
