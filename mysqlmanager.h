...
public:
    ...
    std::vector<Guide> fetchGuides() {
        ...
        QSqlQuery query(
            "SELECT guide_id, name, license_number, languages, experience_years, rating, contact FROM Guides"
            );
        while (query.next()) {
    ...
    std::vector<Guide> fetchAvailableGuides() {
        ...
        QSqlQuery query(
            "SELECT guide_id, name, license_number, languages, experience_years, rating, contact "
    ...
    // either way.
    bool assignGuide(int bookingId, int guideId, QString &message) {
        if (!connect()) {
        ...
        QSqlQuery query;
        query.prepare("CALL sp_assign_guide(?, ?, @success, @message)");
        ...
        QSqlQuery result;
        result.exec("SELECT @success, @message");
        if (result.next()) {
