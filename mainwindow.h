...
struct TripPlan {
    ...
    double totalCost = 0;
    // The budget limit typed into Budget Calculator at the time of the
    // last calculation (0 = no limit was set). Kept around so booking
    // can still enforce it later, even after leaving the Budget
    // Calculator tab.
    double budgetLimit = 0;
    // Total NPR added by any guide(s) assigned to this trip's booking -
    // kept separate from totalCost's other components so the trip bar
    // and Profile summary can show "guide fee included" explicitly.
    double guideFee = 0;
