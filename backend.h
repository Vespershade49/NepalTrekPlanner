...
#include "itinerary.h"            // matches the proposal's ER diagram
#include <vector>
#include <map>
#include <string>
#include <algorithm>
...
public:
    ...
    vector<Itinerary>                 itineraries;      // matches ER diagram
    ...
    // Filter spots by max entry fee
    vector<TouristSpot> filterSpotsByBudget(double maxFee) {
        vector<TouristSpot> results;
        for (TouristSpot spot : touristSpots)
            if (spot.entryFee <= maxFee)
                results.push_back(spot);
        return results;
    }

    // Filter trek routes by difficulty
    vector<TrekRoute> filterTreksByDifficulty(string difficulty) {
        vector<TrekRoute> results;
        for (TrekRoute route : trekRoutes)
    ...
    // ── Trek Recommendation ───────────────────────────────────
    // Returns exact matches first; falls back to day+budget match
    vector<TrekRoute> recommendRoutes(UserProfile user) {
        vector<TrekRoute> exact;
        vector<TrekRoute> fallback;

        for (TrekRoute route : trekRoutes) {
            bool daysOk    = route.daysRequired   <= user.availableDays;
            bool budgetOk  = route.estimatedCost  <= user.budget;
            bool fitnessOk = route.difficulty     == user.fitnessLevel;

            if (daysOk && budgetOk && fitnessOk)
                exact.push_back(route);
            else if (daysOk && budgetOk)
                fallback.push_back(route);
        }
        return exact.empty() ? fallback : exact;
    }
    ...
    BudgetCalculator::CostBreakdown calculateBudget(
        ...
        int numberOfPeople)
    {
        return budgetCalc.getBreakdown(route, spots, days,
                                       accommodationType, numberOfPeople);
    }

    // ── Itinerary Save/Load (backed by a plain text file) ─────
    // The UI just calls these two functions - it has no idea
    // whether it's a text file or a database underneath.
    ...
    // Matches the proposal's ER diagram exactly:
